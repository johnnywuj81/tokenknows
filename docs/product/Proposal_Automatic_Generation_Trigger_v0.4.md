...
> 2026-05-22 8 条 (Q1-Q8) 全部按推荐方案确认 → v0.4.0-v0.4.4 全周期研发可启动。

| # | 问题 | 状态 | 最终决策 |
| --- | --- | --- | --- |
| Q1 | 5 分钟撤回窗口长度是否可配？ | ✅ **已决（2026-05-22）** | **用户可调 1-15 min**，默认 5 min，最小 1 min；高变动项目可缩短提响应、低变动项目可拉长减误触发 |
| Q2 | 自动归档"7 天未触碰" 这个阈值合理吗？ | ✅ **已决（2026-05-22）** | **7 天**，与企业 OKR / Sprint 周期对齐；Reviewer 有 3 工作日反应 + 4 天 buffer |
| Q3 | 月配额默认值？ | ✅ **已决（2026-05-22）** | **5M token/月**，覆盖 ~50 份长文档；Claude Sonnet 实际费用约 $50-100/月 |
| Q4 | book 类型默认勾选启用吗？ | ✅ **已决（2026-05-22）** | **默认不勾**（token 用量约周报 20 倍）；引导向导显示但默认未勾，用户可在设置里手动开启 |
| Q5 | Skill 自动蒸馏需要 contributor 个人确认吗？ | ✅ **已决（2026-05-22）** | **必须个人确认**；与 v0.3 IM-4 同意书原则一致；满足 PIPL + 企业合作合同条款 |
| Q6 | 实例级默认规则能否被项目级覆盖？ | ✅ **已决（2026-05-22）** | **能（项目级优先）**；项目可自定义 cron 时间等参数，实例级作兜底；T28 seeder 需支持 project_id=NULL 与项目级共存 |
| Q7 | 配额仪表盘对哪些角色可见？ | ✅ **已决（2026-05-22）** | **Owner + 实例管理员**；Editor/Reviewer 不为成本负责，UI 隐藏；管理员可看全部项目用量 |
| Q8 | Skill 自进化检查频率？ | ✅ **已决（2026-05-22）** | **每天 03:00**，与 §7.3 调度任务表一致；避开工作时间；LLM 成本与 UX 感知都适中 |
